...
export default function ExpandableCardDemo() {
  ...
  return <div className="service-cards">
    {cards.map((card, index) => <motion.article layout key={card.title} role="group" initial={{ opacity: 0, y: -110, rotateX: -14, scale: .96 }} whileInView={{ opacity: 1, y: 0, rotateX: 0, scale: 1 }} viewport={{ once: true, amount: .18 }} transition={{ duration: .85, delay: index * .1, ease: [0.16, 1, 0.3, 1] }} whileHover={{ y: -4 }} className={`service-card ${active === index ? 'is-active' : ''}`} onClick={() => setActive(active === index ? null : index)}>
      <div className="service-card-top"><img src={card.src} alt="" /><div><p>{String(index + 1).padStart(2, '0')}</p><h3>{card.title}</h3><span>{card.description}</span></div><button type="button" aria-label={`${active === index ? 'Close' : 'View'} ${card.title}`}>{active === index ? 'Close' : 'View'}</button></div>
      <AnimatePresence initial={false}>{active === index && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="service-card-detail"><p>{card.content}</p><a href="/contact">Discuss this capability &rarr;</a></motion.div>}</AnimatePresence>
    </motion.article>)}
  </div>
}
